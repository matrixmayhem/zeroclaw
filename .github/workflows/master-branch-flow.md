...
| `docker-deploy-on-master.yml` | `push` -> `master` | Build and publish the Dockploy GHCR image |
...
| Push to `master` (including after merge) | `docker-deploy-on-master.yml` |
...
2. `docker-deploy-on-master.yml` starts:
   - `web` job: builds `web/dist`.
   - `build` job (matrix, 2 targets): `x86_64-unknown-linux-gnu` and `aarch64-unknown-linux-gnu`.
   - `docker` job: assembles the pre-built binaries into the Debian CI image and pushes a multi-platform image (`linux/amd64,linux/arm64`) to `ghcr.io`.
   - Published tags: `ghcr.io/matrixmayhem/zeroclaw:edge-debian` and `ghcr.io/matrixmayhem/zeroclaw:sha-<short-sha>-debian`.
3. This runs on every push to `master` without creating beta releases or prerelease tags.
...
3. `build` job (matrix, same 4 targets as before): compiles release binary.
...
| Target | `checks-on-pr.yml` | `cross-platform-build-manual.yml` | `docker-deploy-on-master.yml` | `release-stable-manual.yml` |
| --- | :---: | :---: | :---: | :---: |
| `x86_64-unknown-linux-gnu` | x |  | x | x |
| `aarch64-unknown-linux-gnu` |  | x | x | x |
| `aarch64-apple-darwin` | x |  |  | x |
| `x86_64-apple-darwin` |  | x |  |  |
| `x86_64-pc-windows-msvc` | x | x |  | x |
...
### Deploy Image (on every master push)

```mermaid
flowchart TD
  A["Push to master"] --> B["docker-deploy-on-master.yml"]
  B --> B1["web: build web/dist"]
  B1 --> B2["build: x86_64-linux + aarch64-linux"]
  B2 --> B3["docker: push ghcr.io :edge-debian + sha tag"]
```
...
2. **Deploy image not appearing**: confirm the push landed on `master` (not another branch); check `docker-deploy-on-master.yml` run status and GHCR package visibility.
